...
Page({
  ...
  onShow: function () {
    var init4 = wx.getStorageSync('init4')
    this.get_user_info()
    // 重置人才模型同步普通简历
    // this.onLoad()
    if (init4 == 1) {
      console.log("从重置人才模型过来")
      wx.setStorageSync('init4', 0)
      this.onLoad()
    }
  ...
  details: function (e) {
    ...
    // 判断简历是否是核实过的，核实过跳过校验  普通简历不用校验58
    if (2 == 2) {
    ...
    if (this.data.is_search) {
      ...
      if (this.data.search_g_num < 15) {
        ...
        wx.request({
          ...
          method: "POST",
          url: 'https://www.hr24.com.cn/hunting_ground/ordinary_search_resume',
          data: {
            place_id: place_id,
            place_parent_id: place_parent_id,
            // industry_id: industry_id,
